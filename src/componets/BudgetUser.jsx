    ...
    const fetchResources = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/resources/${project.ProjectID}`
        );
    ...
    const fetchFinancialsAndCalculateCosts = async () => {
      try {
        ...
        usersData.forEach((user) => {
          hourlyRatesMap[user.UserID] = user.HourlyRate;
        });
  ...
  return (
    <div className="flex flex-col p-5 items-center">
      ...
      <ul className="space-y-2">
        ...
        <p className="mt-6 ">Total Net Revenue: R{netRevenue}</p>{" "}
        {/* Display total net revenue in Rands */}
        <p className="mt-2 ">Total Costs: R{totalCosts}</p>{" "}
        {/* Display total costs in Rands */}
      </ul>
    </div>
  );
};
