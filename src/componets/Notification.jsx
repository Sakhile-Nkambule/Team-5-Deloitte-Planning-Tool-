...
function Notification({
  ...
  return (
    <div
      className={`${unreadContainerStyles} mb-10 relative mx-auto w-full max-w-2xl`}
    >
      <div
        className={`${bgColor} w-5/6 rounded-t-lg shadow-2xl p-2 text-black font-bold text-xl min-h-120`}
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <FontAwesomeIcon icon={faBell} className="pr-2 pl-2" size="1x" />
            {/* Apply dynamic styling based on Read_ status */}
            <span className={unreadStyles}>{description}</span>
          </div>
          <FontAwesomeIcon
            icon={faTimes}
      ...
      <div
        className={`${unreadContainerStyles} w-5/6 rounded-b-lg shadow-2xl p-2 mb-10`}
      >
        <div className="text-sm text-gray-600">Created At: {formattedDate}</div>
        <div>
          <span className={unreadStyles}>{messagePart}</span>
